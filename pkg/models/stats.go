...
// ExampleStatsEndpointGrouping demonstrates the endpoint grouping functionality
func ExampleStatsEndpointGrouping() {
	stats := &Stats{}

	// Simulate API calls with UUIDs that should be grouped
	...
	stats.RecordCall("GET", "/labs/a7d20917-5e57-407f-80ea-63596c53f198/nodes", 200, 100*time.Millisecond)
	stats.RecordCall("POST", "/labs/a7d20917-5e57-407f-80ea-63596c53f198/nodes", 201, 120*time.Millisecond)

	// Check that endpoints are properly grouped
	fmt.Printf("Total calls: %d\n", stats.TotalCalls())
	fmt.Printf("GET calls: %d\n", stats.CallsByMethod()["GET"])
	fmt.Printf("POST calls: %d\n", stats.CallsByMethod()["POST"])

	// Check grouped endpoints
	getLabGroup := stats.EndpointGroups["GET /labs/{id}"]
	fmt.Printf("GET /labs/{id} - Calls: %d, Min: %v, Max: %v, Avg: %v\n",
		getLabGroup.CallCount, getLabGroup.MinTime, getLabGroup.MaxTime, getLabGroup.AvgTime)

	getNodesGroup := stats.EndpointGroups["GET /labs/{id}/nodes"]
	fmt.Printf("GET /labs/{id}/nodes - Calls: %d, Min: %v, Max: %v, Avg: %v\n",
		getNodesGroup.CallCount, getNodesGroup.MinTime, getNodesGroup.MaxTime, getNodesGroup.AvgTime)

	postNodesGroup := stats.EndpointGroups["POST /labs/{id}/nodes"]
	fmt.Printf("POST /labs/{id}/nodes - Calls: %d, Min: %v, Max: %v, Avg: %v\n",
		postNodesGroup.CallCount, postNodesGroup.MinTime, postNodesGroup.MaxTime, postNodesGroup.AvgTime)

	// Output:
	// Total calls: 4
	// GET calls: 3
	// POST calls: 1
	// GET /labs/{id} - Calls: 2, Min: 50ms, Max: 75ms, Avg: 62.5ms
	// GET /labs/{id}/nodes - Calls: 1, Min: 100ms, Max: 100ms, Avg: 100ms
	// POST /labs/{id}/nodes - Calls: 1, Min: 120ms, Max: 120ms, Avg: 120ms
}
